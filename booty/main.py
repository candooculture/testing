...
def render_report_html(data, ors_result):
    ...
    <body style='font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto; padding: 20px; color: #333;'>
      ...
      <ul>
        <li><strong>Total Risk Impact:</strong> ${ors_result.get("total_risk", "N/A")}</li>
        <li><strong>Estimated EBITDA Margin:</strong> {ors_result.get("ebitda_margin", "N/A")}%</li>
        <li><strong>EBITDA at Risk:</strong> {ors_result.get("ebitda_risk_percent", "N/A")}%</li>
      </ul>

      <h3>🧾 Risk Breakdown</h3>
      <ul>
        <li>Payroll Waste: ${ors_result.get("payroll_loss", "N/A")}</li>
        <li>Customer Churn: ${ors_result.get("churn_loss", "N/A")}</li>
        <li>Leadership Drag: ${ors_result.get("leadership_loss", "N/A")}</li>
        <li>Workforce Productivity: ${ors_result.get("workforce_loss", "N/A")}</li>
        <li>Process Gaps: ${ors_result.get("deep_dive_loss", "N/A")}</li>
      </ul>

      <h3>🧠 Inputs Summary</h3>
      <ul>
        <li>Employees: {data.get("total_employees", "N/A")}</li>
        <li>Average Salary: ${data.get("avg_salary", "N/A")}</li>
        <li>Total Revenue: ${data.get("total_revenue", "N/A")}</li>
        <li>Industry: {data.get("industry", "N/A")}</li>
      </ul>
