...
def render_report_html(data):
    ...
    <body style='font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto; padding: 20px; color: #eee; background-color: #1a1a1a;'>
      ...
      <ul>
        <li><strong>Employees:</strong> {data.get("total_employees", "N/A")} <em>(user-entered)</em></li>
        <li><strong>Average Salary:</strong> {format_dollars(data.get("avg_salary"))} <em>(user-entered)</em></li>
        <li><strong>Total Payroll Cost:</strong> {format_dollars(data.get("payroll_cost"))} <em>(calculated)</em></li>
      ...
      <p style='margin-top: 30px; font-size: 13px; color: #666;'>Generated by the Candoo Culture ORS Engine.</p>
    </body>
    </html>
    """
